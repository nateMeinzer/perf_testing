...
select case when (select count(*) 
                  from store_sales 
                  where ss_quantity between 1 and 20) > 31000
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 21 and 40) > 4078
            then (select avg(ss_ext_tax)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 41 and 60) > 28395
            then (select avg(ss_ext_tax)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 61 and 80) > 12716
            then (select avg(ss_ext_tax)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 81 and 100) > 41806
            then (select avg(ss_ext_tax)
                  from store_sales
...
where ss_item_sk = i_item_sk and
      ...
      ss_store_sk = s_store_sk and
      d_year in (2002) and
        ((i_category in ('Men','Books','Children') and
          i_class in ('sports-apparel','self-help','infants')
         )
      or (i_category in ('Sports','Electronics','Shoes') and
          i_class in ('athletic shoes','memory','athletic') 
        ))
...
where 
	...
	and d_date between cast('2000-05-01' as date) 
				and DATE_ADD(cast('2000-05-01' as date), 30)
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
...
with ss as
 ...
 where ss_sold_date_sk = d_date_sk
       and d_date between cast('1998-08-20' as date) 
                  and DATE_ADD(cast('1998-08-20' as date), 30) 
       and ss_store_sk = s_store_sk
 group by s_store_sk)
 ,
 sr as
...
select  c_customer_id as customer_id
 ...
 where ca_city	        =  'Hillcrest'
   and c_current_addr_sk = ca_address_sk
   and ib_lower_bound   >=  65353
   and ib_upper_bound   <=  65353 + 50000
...
with ssr as
 ...
 where date_sk = d_date_sk
       and d_date between cast('1999-08-28' as date) 
                  and DATE_ADD(cast('1999-08-28' as date), 14)
