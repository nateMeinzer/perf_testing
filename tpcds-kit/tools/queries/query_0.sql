...
select top 100 
 ...
 where
  ...
  ca_county in ('Clay County','Iron County','Lampasas County','Perry County','Johnson County') and
...
select top 100 i_item_id,
 ...
 where cs_sold_date_sk = d_date_sk and
       ...
       c_birth_month in (11,5,10,9,12,6) and
       d_year = 1998 and
...
select top 100 i_item_id
 ...
 and d_date between cast('1999-06-13' as date) 
 				and (cast('1999-06-13' as date) + 30 days)
 group by i_item_id
         ,i_item_desc 
         ,i_category
         ,i_class
...
with ss as (
 ...
 where i_item_id in (select
     i_item_id
from item
where i_color in ('almond','blush','lemon'))
 and     ss_item_sk              = i_item_sk
 and     ss_sold_date_sk         = d_date_sk
 and     d_year                  = 2001
 and     d_moy                   = 6
...
cross_sales as
 (select i_product_name product_name
  ...
  WHERE  ss_store_sk = s_store_sk AND
         ...
         i_color in ('navy','lavender','lace','midnight','burlywood','saddle') and
...
select top 100  
 ...
 from (
 	...
 	and ws_ship_mode_sk = sm_ship_mode_sk
        and d_year = 1998
 	and t_time between 11416 and 11416+28800 
...
select top 100 i_item_id
 ...
 and i_manufact_id in (580,608,931,483)
 and inv_quantity_on_hand between 100 and 500
 and ss_item_sk = i_item_sk
 group by i_item_id,i_item_desc,i_current_price
 order by i_item_id
...
with sr_items as
 ...
 and   d_date    in 
	...
	where d_week_seq in 
		(select d_week_seq
		from date_dim
	  where d_date in ('1999-04-02','1999-08-18','1999-11-23')))
...
select top 100 c_customer_id as customer_id
 ...
 where ca_city	        =  'Hamilton'
   and c_current_addr_sk = ca_address_sk
   and ib_lower_bound   >=  5183
   and ib_upper_bound   <=  5183 + 50000
...
where ss_item_sk = i_item_sk and
      ...
      d_year in (2002) and
        ((i_category in ('Home','Electronics','Women') and
          i_class in ('decor','memory','dresses')
         )
      or (i_category in ('Children','Men','Sports') and
          i_class in ('school-uniforms','sports-apparel','fishing') 
        ))
